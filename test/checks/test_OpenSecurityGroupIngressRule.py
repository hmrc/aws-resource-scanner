...
from aws_resource_scanner.checks.OpenSecurityGroupIngressRule import check, OpenSecurityGroupIngressRule
...
def test_new_violation():
    ...
    assert isinstance(violation.rule, OpenSecurityGroupIngressRule)
    assert violation.message == f"Security group {violation.resource['GroupName']} ({violation.resource['GroupId']}) ingress rule allows traffic from anywhere."
    assert violation.resource['GroupName'] == 'test_new_violation'
